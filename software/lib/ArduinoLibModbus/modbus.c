...
static void _sleep_response_timeout(modbus_t *ctx)
{
    ...
    Sleep(ctx->response_timeout);
#elif defined(ARDUINO)
    delay(ctx->response_timeout);
#else
    /* usleep source code */
    struct timespec request, remaining;
    request.tv_sec = ctx->response_timeout * 1000;
    request.tv_nsec = 0;
                          ...
                          uint16_t *dest)
{
    ...
    // Request requires 8 (slave+func+addr+nb+crc)
    // Response requires 5+2*nb (slave+func+bytes+values+crc), and 6 of request is retained for validation (slave+func+addr+nb)
    uint8_t req[5 + 2 * MODBUS_MAX_READ_REGISTERS + 6];
    uint8_t *rsp = req + 6;
...
// int modbus_write_registers(modbus_t *ctx, int addr, int nb, const uint16_t *src)
{
    ...
    int req_length;
    int byte_count;
    // Request requires 9+2*nb (slave+func+addr+nb+bytes+values+crc)
    // Response requires 8 (slave+func+addr+nb+crc), and 6 of request is retained for validation (slave+func+addr+nb)
    uint8_t req[(MODBUS_MAX_WRITE_REGISTERS < 3) ? 14 : (2 * MODBUS_MAX_WRITE_REGISTERS + 9)];
    uint8_t *rsp = req + 6;
...
int modbus_set_slave(modbus_t *ctx, uint8_t slave)
{
    if (ctx == NULL)
    {
        errno = EINVAL;
...
int modbus_get_response_timeout(modbus_t *ctx, uint16_t *ms)
{
    if (ctx == NULL)
    {
        errno = EINVAL;
    ...
    *ms = ctx->response_timeout;
    return 0;
}

int modbus_set_response_timeout(modbus_t *ctx, uint16_t ms)
{
    if (ctx == NULL || ms == 0)
    {
        errno = EINVAL;
    ...
    ctx->response_timeout = ms;
    return 0;
}

/* Get the timeout interval between two consecutive bytes of a message */
int modbus_get_byte_timeout(modbus_t *ctx, uint16_t *ms)
{
    if (ctx == NULL)
    {
        errno = EINVAL;
...
int modbus_set_byte_timeout(modbus_t *ctx, uint16_t ms)
{
    /* Byte timeout can be disabled when both values are zero */
    if (ctx == NULL || ms > 999999)
    {
        errno = EINVAL;
        return -1;
    }
