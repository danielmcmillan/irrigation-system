...
static int _modbus_set_slave(modbus_t *ctx, uint8_t slave)
{
...
static int _modbus_rtu_select(modbus_t *ctx, fd_set *rset,
                              uint16_t tv, int length_to_read)
{
    int s_rc;
#if defined(_WIN32)
...
#endif
{
    ...
    uint8_t *data = (uint8_t *)malloc(sizeof(modbus_t) + sizeof(modbus_rtu_t));
    ctx = (modbus_t *)data;
    _modbus_init_common(ctx);
    ctx->backend = &_modbus_rtu_backend;
    ctx->backend_data = (modbus_rtu_t *)(data + sizeof(modbus_t));
    ctx_rtu = (modbus_rtu_t *)ctx->backend_data;
#ifdef ARDUINO
    ctx_rtu->rs485 = rs485;
#else
